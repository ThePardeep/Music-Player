...
class BuildSong extends StatelessWidget {
  ...
  Function playSong, insertToQueueUsingPath;

  BuildSong(this._songList);
  ...
  void insertToQueue(index) {
    ...
    this.insertToQueueUsingPath(_songList[index]['path']);
  }
    ...
    return Container(
      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton(
            ...
            onSelected: (action) {
              if (action == "PlayNow") {
                this.playSong(_songList[index]['path']);
              }
            }),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return ScopedModelDescendant<SongsModel>(
      builder: (BuildContext context, Widget child, SongsModel model) {
        ...
        this.playSong = model.playDirectWithPath;
